...
%
%  Dynamics:
%     Z⁺ = A10 Z + B10 u + C10     (100→10 Hz compression)
...
%     model.C,model.Cc1,model.K_obs,model.mean_std_out,2000);
% figure(1)
% plot(yout(:,1),ypred(:,2),'LineWidth',4);hold on;
% plot(ypred(:,1),ypred(:,2),'LineWidth',4); grid on;
% clc;
% save('traj_102_v2.mat','-v7.3')
%%
load("traj_102_v2.mat")
...
%% Sampling MPC at 10 hz and koopman at 100hz
...
C10  = S * Bc1;      % effective constant term
...
% === COST WEIGHTS ===
Q   = diag([10 10 5 10 0 0]);   % tracking cost for [X,Y,psi,u,v,r]
Ru  = diag([1e-2 1e-4]);             % input effort
Rdu = diag([1 0.01]);
...
for k = 1:Np
    ...
    opti.subject_to( Z(:,k+1) == A10 * Z(:,k) + B10*uk + C10 );
...
for MPC_iter = 1:block:TotalSteps
    % MPC_iter
    rows_idx = MPC_iter + (0:Np-1);
    per_rows_idx = MPC_iter + (0:Np-1);
    ...
    plot(rows_idx*dt_mpc,sol.value(U(2,:)),'LineWidth',lw); hold on;
    ...
    plot(rows_idx*dt_mpc,sol.value(U(1,:)),'LineWidth',lw); hold on;
    plot(size(usol,2)*dt_mpc,usol(1,:),'LineWidth',lw); grid on;
    xlabel('Time [s]')
...
subplot(2,2,1)
plot(Z_hc(:,1),Z_hc(:,2),'LineWidth',lw); hold on;
plot(xsol(:,1),xsol(:,2),'LineWidth',lw); grid on;
xlabel('X')
ylabel('Y')
box on;
set(gca, 'LineWidth', 1.5)
