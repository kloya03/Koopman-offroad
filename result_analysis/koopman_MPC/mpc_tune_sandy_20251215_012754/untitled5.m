...
models{1}.filename = "Ksandy_on_clay_MPC_20_3.avi";
models{2}.filename = "Kclay_on_clay_MPC_20_3.avi";
...
for model_iter = length(models)
    ...
    % === COST WEIGHTS ===
    Q   = diag([10 10 10 1 15 15]);   % tracking cost for [X,Y,psi,u,v,r]
    ...
    for MPC_iter = 1:block:TotalSteps
        ...
        plotControlsMPC(axU1, axU2, ...
            ...
            usol, lw);
        % --- inside your loop ---
        plotTrajWithCar(axTraj, Data.OutputData, yfull, x0, xsol, 9, 4);
...
for jj=1:3
    ...
    plot(tspan,models{1}.xsol(:,nk),cc{1},'LineWidth',lw); hold on;
    plot(tspan,models{2}.xsol(:,nk),cc{2},'LineWidth',lw); grid on;
    xlabel('Time [s]')
...
for jj=1:2
    ...
    plot(tspan,models{1}.usol(jj,:),cc{1},'LineWidth',lw); hold on;
    plot(tspan,models{2}.usol(jj,:),cc{2},'LineWidth',lw); grid on;
    xlabel('Time [s]')
...
if needCreate
    ...
    ud.p_out   = plot(ax, outXY(:,1), outXY(:,2), 'LineWidth', lw);
