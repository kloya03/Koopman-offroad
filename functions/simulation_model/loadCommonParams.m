...
b.W = 1.511;
b.H = 1.875;
b.wb = 2.719;                          % wheelbase
b.Lf = b.wb/2;                         % length (front)
b.Lr = b.wb/2;                         % length (rear)
b.Iy = 1/12*b.m*(b.L^2 + b.H^2);
b.Iz = 1/12*b.m*(b.L^2 + b.W^2);
b.g = 9.81;                          % Gravity
...
b.rpm_vals = [50, 200, 300, 350, 400, 450, 550, 650, 700];
% Corresponding torque values (in Newton-meters)
b.tau_vals = [90, 105, 110, 120, 125, 130, 135, 130,  120];
b.rad_per_sec = 2*pi*b.rpm_vals/60;
b.speeds = b.rad_per_sec*0.33;
engine_speeds = linspace(0, b.rad_per_sec(end)+20, 500);
b.tau_curve = @(engine_speeds) max(interp1(b.rad_per_sec, b.tau_vals, engine_speeds , 'spline', 'extrap'),0);
b.estimated_torque = b.tau_curve(engine_speeds);
...
b.hD = randn(b.N,b.N);
b.wf = 2;
b.hkx = (1:b.N)' * pi * b.wf / b.Lx;
b.hky = (1:b.N)  * pi * b.wf / b.Ly;

% Terrain height function: normalized to max |height| = H
raw_terrain = @(x, y) arrayfun(@(xi, yi) ...
    sum(sum( ...
        b.hA .* cos(b.hkx * xi) .* cos(b.hky * yi) + ...
        b.hB .* sin(b.hkx * xi) .* cos(b.hky * yi) + ...
        b.hC .* cos(b.hkx * xi) .* sin(b.hky * yi) + ...
        b.hD .* sin(b.hkx * xi) .* sin(b.hky * yi) ...
    )), x, y);

% Evaluate raw terrain on a reference grid to find its max
[b.hxref, b.hyref] = meshgrid(linspace(0, b.Lx, 100), linspace(0, b.Ly, 100));
b.hzref = raw_terrain(b.hxref, b.hyref);
b.hzmax = max(abs(b.hzref(:)));   % Maximum absolute height
% Final height function scaled to max height H
b.h = @(x, y) b.H * raw_terrain(x, y) / b.hzmax;

b.dh_x = @(x, y) b.H / b.hzmax * arrayfun(@(xi, yi) ...
    sum(sum( ...
        -b.hA .* (b.hkx .* sin(b.hkx * xi)) .* cos(b.hky * yi) + ...
         b.hB .* (b.hkx .* cos(b.hkx * xi)) .* cos(b.hky * yi) - ...
         b.hC .* (b.hkx .* sin(b.hkx * xi)) .* sin(b.hky * yi) + ...
         b.hD .* (b.hkx .* cos(b.hkx * xi)) .* sin(b.hky * yi) ...
    )), x, y);

b.dh_y = @(x, y) b.H/ b.hzmax * arrayfun(@(xi, yi) ...
    sum(sum( ...
        -b.hA .* cos(b.hkx * xi) .* (b.hky .* sin(b.hky * yi)) - ...
         b.hB .* sin(b.hkx * xi) .* (b.hky .* sin(b.hky * yi)) + ...
         b.hC .* cos(b.hkx * xi) .* (b.hky .* cos(b.hky * yi)) + ...
         b.hD .* sin(b.hkx * xi) .* (b.hky .* cos(b.hky * yi)) ...
    )), x, y);

b.ddh_xx = @(x, y) b.H/ b.hzmax * arrayfun(@(xi, yi) ...
    sum(sum( ...
        -b.hA .* (b.hkx.^2 .* cos(b.hkx * xi)) .* cos(b.hky * yi) - ...
         b.hB .* (b.hkx.^2 .* sin(b.hkx * xi)) .* cos(b.hky * yi) - ...
         b.hC .* (b.hkx.^2 .* cos(b.hkx * xi)) .* sin(b.hky * yi) - ...
         b.hD .* (b.hkx.^2 .* sin(b.hkx * xi)) .* sin(b.hky * yi) ...
    )), x, y);

b.ddh_yy = @(x, y) b.H/ b.hzmax * arrayfun(@(xi, yi) ...
    sum(sum( ...
        -b.hA .* cos(b.hkx * xi) .* (b.hky.^2 .* cos(b.hky * yi)) - ...
         b.hB .* sin(b.hkx * xi) .* (b.hky.^2 .* cos(b.hky * yi)) - ...
         b.hC .* cos(b.hkx * xi) .* (b.hky.^2 .* sin(b.hky * yi)) - ...
         b.hD .* sin(b.hkx * xi) .* (b.hky.^2 .* sin(b.hky * yi)) ...
    )), x, y);

b.ddh_xy = @(x, y) b.H/ b.hzmax * arrayfun(@(xi, yi) ...
    sum(sum( ...
         b.hA .* (b.hkx .* sin(b.hkx * xi)) .* (b.hky .* sin(b.hky * yi)) - ...
         b.hB .* (b.hkx .* cos(b.hkx * xi)) .* (b.hky .* sin(b.hky * yi)) + ...
         b.hC .* (b.hkx .* sin(b.hkx * xi)) .* (b.hky .* cos(b.hky * yi)) - ...
         b.hD .* (b.hkx .* cos(b.hkx * xi)) .* (b.hky .* cos(b.hky * yi)) ...
    )), x, y);
